...
When optimizing the stratification of a sampling frame, values of the target variables Y's are available for the generality of the units in the frame, or at least for a sample of them by means of which it is possible to estimate means and standard deviation of Y's in atomic strata. 
...
that is, we replicate twice the variable **GDP** because it will be used once for **infant mortality** and once for **contraception**. We set 10% and 5% precision constraints on these two variables.
